...
(() => {
  ...
  var MAX_LIVES = 10;
  var EQUIPPED_WEAPON_SIZE = 92;
  var EQUIPPED_WEAPON_OFFSET = 24;
  var SHIELD_DURABILITY = 3;
  var WEAPON_CONFIGS = {
    assaultRifle: {
      kind: "ranged",
      aimDelay: 0.65,
  ...
  var noiseBufferCache = /* @__PURE__ */ new Map();
  ...
  function getNoiseBuffer(audio, duration) {
    const cacheKey = Math.round(duration * 1e3);
  ...
  function playShotSound(type) {
    ...
    const settings = {
      assaultRifle: { frequency: 135, duration: 0.065, volume: 0.09 },
      uzi: { frequency: 190, duration: 0.04, volume: 0.065 },
      pistol: { frequency: 115, duration: 0.11, volume: 0.13 },
      shotgun: { frequency: 75, duration: 0.18, volume: 0.18 }
    };
    ...
    oscillatorGain.gain.exponentialRampToValueAtTime(1e-3, now + setting.duration);
    ...
    noiseGain.gain.setValueAtTime(setting.volume * (type === "shotgun" ? 1.5 : 0.65), now);
    noiseGain.gain.exponentialRampToValueAtTime(1e-3, now + setting.duration);
  ...
  function playPickupReminderSound() {
    ...
    for (const [index, frequency] of [660, 880].entries()) {
      ...
      gain.gain.exponentialRampToValueAtTime(1e-3, start + 0.16);
  ...
  function playHealSound() {
    ...
    for (const [index, frequency] of [523, 659, 784].entries()) {
      ...
      gain.gain.exponentialRampToValueAtTime(1e-3, start + 0.22);
      oscillator.connect(gain).connect(audio.destination);
      oscillator.start(start);
  ...
  function playDamageSound() {
    ...
    oscillator.frequency.exponentialRampToValueAtTime(52, now + 0.12);
    gain.gain.setValueAtTime(0.12, now);
    gain.gain.exponentialRampToValueAtTime(1e-3, now + 0.13);
    oscillator.connect(gain).connect(audio.destination);
    oscillator.start(now);
    oscillator.stop(now + 0.13);
    const noise = audio.createBufferSource();
    const noiseGain = audio.createGain();
    noise.buffer = getNoiseBuffer(audio, 0.07);
  ...
  function createBalls(count, mode = "classic") {
    ...
    for (const [index, preset] of BALL_PRESETS.slice(0, count).entries()) {
      ...
      let attempts = 0;
      while (createdBalls.some((ball) => circlesOverlap(position, radius + 55, ball.position, ball.radius)) && attempts < 100) {
        position = randomPointInsideArena(radius + 45);
        attempts += 1;
      }
      const movementAngle = Math.random() * Math.PI * 2;
      const speed = isBoss ? randomBetween(260, 340) : mode === "chaos" ? randomBetween(500, 650) : randomBetween(330, 450);
      createdBalls.push({
        ...preset,
        team: mode === "teams" ? index % 2 === 0 ? "red" : "blue" : mode === "boss" ? isBoss ? "red" : "blue" : null,
        position,
        velocity: {
  ...
  var enabledWeaponTypes = [...WEAPON_TYPES];
  var healingEnabled = true;
  var gameMode = "classic";
  var weaponPickups = [];
  var heartPickups = [];
  var projectiles = [];
  var WEAPON_RADIUS = 46;
  var MAX_WEAPON_PICKUPS = 6;
  var MIN_SPAWN_DELAY = 1;
  var MAX_SPAWN_DELAY = 2.5;
  var nextWeaponId = 1;
  var nextHeartId = 1;
  var nextProjectileId = 1;
  var weaponSpawnTimer = randomBetween(MIN_SPAWN_DELAY, MAX_SPAWN_DELAY);
  var pickupReminderTimer = 0;
  var heartSpawnTimer = randomBetween(4, 7);
  function randomBetween(minimum, maximum) {
    return minimum + Math.random() * (maximum - minimum);
  }
  function getWeaponSpawnDelay() {
    return gameMode === "chaos" ? randomBetween(0.3, 0.7) : randomBetween(MIN_SPAWN_DELAY, MAX_SPAWN_DELAY);
  }
  ...
  function updateHeartPickups(deltaTime) {
    ...
    if (heartSpawnTimer <= 0 && heartPickups.length < 2) {
      ...
      heartSpawnTimer = gameMode === "chaos" ? randomBetween(2, 4) : randomBetween(5, 9);
    }
    for (let index = heartPickups.length - 1; index >= 0; index--) {
      ...
      const collector = balls.find((ball) => {
        ...
        const weaponCenter = ball.weapon ? getEquippedWeaponCenter(ball, ball.weapon.angle) : null;
        const weaponTouchesHeart = weaponCenter ? circlesOverlap(weaponCenter, EQUIPPED_WEAPON_SIZE * 0.6, heart.position, heart.radius) : false;
        return ball.lives > 0 && ball.lives < ball.maxLives && (bodyTouchesHeart || weaponTouchesHeart);
      });
      if (collector) {
  ...
  function isOpponent(owner, candidate) {
    return candidate.id !== owner.id && candidate.lives > 0 && !(isTeamBasedMode() && owner.team !== null && candidate.team === owner.team);
  }
  function chooseNearestTarget(owner) {
    const candidates = balls.filter((ball) => ball.id !== owner.id && isOpponent(owner, ball));
  ...
  function createEquippedWeapon(owner, type, fallbackAngle) {
    ...
    return {
      type,
      angle: target ? angleBetween(owner.position, target.position) : fallbackAngle,
      ...
      targetRefreshTimer: randomBetween(0.8, 1.4)
    };
  }
  function collectWeaponPickups() {
    for (let index = weaponPickups.length - 1; index >= 0; index--) {
      ...
      const collector = balls.find((ball) => ball.lives > 0 && circlesOverlap(ball.position, ball.radius, pickup.position, pickup.radius));
      if (!collector) {
        continue;
      }
  ...
  function updateWeaponAiming(deltaTime) {
    for (const owner of balls) {
      ...
      let target = weapon.targetId === null ? null : balls.find((ball) => ball.id === weapon.targetId) ?? null;
      if (!target || !isOpponent(owner, target) || weapon.targetRefreshTimer <= 0) {
        ...
        weapon.targetRefreshTimer = randomBetween(0.8, 1.4);
      }
      if (target) {
  ...
  function updateRangedWeapons(deltaTime) {
    for (const owner of balls) {
      ...
      while (owner.weapon === weapon && weapon.phase === "firing" && weapon.timer <= 0) {
  ...
  function getKnifeBladeSegment(ball, angle) {
    ...
    return {
      start: {
        x: ball.position.x + Math.cos(angle) * bladeStartDistance,
        y: ball.position.y + Math.sin(angle) * bladeStartDistance
      },
      end: {
        x: ball.position.x + Math.cos(angle) * bladeEndDistance,
        y: ball.position.y + Math.sin(angle) * bladeEndDistance
      }
    };
  }
  function getWeaponTarget(owner, weapon) {
    ...
    return balls.find((ball) => ball.id === weapon.targetId && ball.id !== owner.id && ball.lives > 0) ?? null;
  }
  ...
  function updateContactWeapons(deltaTime) {
    for (const owner of balls) {
      ...
      if (config.kind === "knife") {
        ...
        for (const candidate of balls) {
          ...
          if (hitTime !== null && hitTime < earliestHitTime) {
            struckBall2 = candidate;
            earliestHitTime = hitTime;
          }
        }
        if (struckBall2) {
          if (struckBall2.weapon?.type === "shield") {
            damageShield(struckBall2, 1);
          } else {
            applyDamage(struckBall2, config.contactDamage);
          }
          owner.weapon = null;
        }
        continue;
      ...
      let nearestDistanceSquared = Number.POSITIVE_INFINITY;
      for (const candidate of balls) {
        if (!isOpponent(owner, candidate) || !circlesOverlap(shieldCenter, config.hitRadius, candidate.position, candidate.radius)) {
          continue;
        }
  ...
  function chooseEarlierProjectileHit(current, candidate) {
    ...
    if (Math.abs(timeDifference) <= 1e-9 && getProjectileHitPriority(candidate) < getProjectileHitPriority(current)) {
      return candidate;
    }
    return current;
  }
  function getArenaExitTime(projectile) {
    ...
    const b = 2 * (startX * directionX + startY * directionY);
    const c = startDistanceSquared - limitSquared;
    const discriminant = Math.max(0, b * b - 4 * a * c);
    const exitTime = (-b + Math.sqrt(discriminant)) / (2 * a);
  ...
  function getEarliestProjectileHit(projectile) {
    ...
    for (const target of balls) {
      if (target.id === projectile.ownerId || target.lives <= 0 || isTeamBasedMode() && owner?.team !== null && target.team === owner?.team) {
        continue;
      }
  ...
  function resolveProjectileCollisions() {
    while (true) {
      ...
      for (const projectile2 of projectiles) {
        const hit2 = getEarliestProjectileHit(projectile2);
        if (hit2 && (!nextCollision || hit2.time < nextCollision.hit.time - 1e-9 || Math.abs(hit2.time - nextCollision.hit.time) <= 1e-9 && projectile2.id < nextCollision.projectile.id)) {
          nextCollision = { projectile: projectile2, hit: hit2 };
        }
      }
      if (!nextCollision) {
        return;
  ...
  var canvas = getCanvas("#game");
  var ctx = getContext2D(canvas);
  var WORLD_WIDTH = 1080;
  var WORLD_HEIGHT = 1920;
  var MOBILE_MODE = window.matchMedia("(pointer: coarse) and (max-width: 900px)").matches;
  var RENDER_SCALE = MOBILE_MODE ? 2 / 3 : 1;
  ...
  var setupPanel = document.querySelector("#setup");
  var battleTypeLabel = document.querySelector("#battle-type");
  var modeInputs = document.querySelectorAll('input[name="mode"]');
  var fighterInputs = document.querySelectorAll('input[name="fighters"]');
  var weaponInputs = document.querySelectorAll("input[data-weapon]");
  var paused = true;
  ...
  function syncBattleModeControls() {
    const selectedMode = document.querySelector('input[name="mode"]:checked')?.value ?? "classic";
  ...
  startButton?.addEventListener("click", () => {
    ...
    gameMode = document.querySelector('input[name="mode"]:checked')?.value ?? "classic";
    const selectedCount = Math.max(2, Math.min(6, Number(selected?.value ?? 2)));
    const count = gameMode === "teams" && selectedCount % 2 !== 0 ? Math.min(6, selectedCount + 1) : selectedCount;
  ...
  function drawFace(ball) {
    ctx.save();
    const eyeOffset = ball.radius * 0.32;
    const eyeY = ball.position.y - ball.radius * 0.12;
    ctx.strokeStyle = "#fff";
    ctx.lineWidth = 6;
    ctx.lineCap = "round";
    if (ball.hurtTimer > 0) {
    ...
    if (ball.weapon) {
      ...
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(ball.position.x, ball.position.y + 29, 18, Math.PI + 0.18, Math.PI * 2 - 0.18);
      ctx.stroke();
    } else {
      ctx.beginPath();
      ctx.arc(ball.position.x, ball.position.y + 10, 18, 0.15, Math.PI - 0.15);
      ctx.stroke();
    }
    ctx.restore();
  }
  ...
  function drawEquippedWeapon(ball) {
    ...
    if (ball.weapon.type === "shield" && ball.weapon.durability !== null) {
      ctx.save();
  ...
  function render() {
    ...
    if (winner || winningTeam) {
      const victoryText = winner ? `${winner.name} WINS` : gameMode === "boss" ? winningTeam === "red" ? "BOSS WINS" : "CHALLENGERS WIN" : `${winningTeam?.toUpperCase()} TEAM WINS`;
      ctx.save();
      ctx.fillStyle = "rgba(3, 4, 12, .76)";
      ctx.fillRect(0, 0, WORLD_WIDTH, WORLD_HEIGHT);
      ctx.textAlign = "center";
      ctx.fillStyle = winner ? winner.color : winningTeam === "red" ? "#ff466f" : "#5d8cff";
      ctx.font = "900 76px Arial";
      ctx.fillText(victoryText, 540, 930);
      ctx.fillStyle = "#fff";
      ctx.font = "600 28px Arial";
      ctx.fillText("Click \xABNew battle\xBB to play again", 540, 990);
      ctx.restore();
    }
  }
  ...
  var FIXED_STEP = MOBILE_MODE ? 1 / 60 : 1 / 120;
  var previousTime = performance.now();
  var accumulator = 0;
  function gameLoop(currentTime) {
    const frameTime = Math.min((currentTime - previousTime) / 1e3, 0.1);
...
//# sourceMappingURL=ball-battles.js.map
